...
@ConditionalOnProperty(name = "app.mock.twitch-category", havingValue = "false", matchIfMissing = true)
public class TwitchCategoryServiceImpl implements TwitchCategoryService {
    ...
    @SuppressWarnings("unchecked")
    private Map<String, Object> callTwitch(String uri, String token) {
    ...
    private static TwitchCategoryCache toCacheEntry(Map<String, Object> g, boolean includeIgdbId) {
    ...
    @SuppressWarnings("unchecked")
    private List<TwitchCategory> fetchFromSearchAndStore(String query) {
        String token = getOrRefreshAppToken();
        if (token.isBlank() || twitchClientId.isBlank()) {
