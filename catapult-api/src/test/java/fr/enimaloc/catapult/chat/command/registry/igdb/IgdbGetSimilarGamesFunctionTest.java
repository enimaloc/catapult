...
class IgdbGetSimilarGamesFunctionTest {

    @Test
    void invokeAcceptsABareIgdbId() throws Exception {
        ChatCommandServiceGateway gateway = mock(ChatCommandServiceGateway.class);
        when(gateway.igdbSimilarGames("1234")).thenReturn(Optional.of(List.of("Counter-Strike 2")));

        IgdbGetSimilarGamesFunction fn = new IgdbGetSimilarGamesFunction(gateway);
        assertThat(fn.namespace()).isEqualTo("igdb");
        assertThat(fn.name()).isEqualTo("getSimilarGames");
        assertThat(fn.parameterNames()).containsExactly("game");

        assertThat(fn.invoke(null, new Object[]{"1234"})).isEqualTo(List.of("Counter-Strike 2"));
    }
    ...
    void invokeAcceptsTheGetGameResultObject() throws Exception {
        ...
        assertThat(fn.invoke(null, new Object[]{gameResult})).isEqualTo(List.of("Counter-Strike 2"));
    }

    @Test
    void invokeReturnsEmptyListWhenGatewayFindsNothing() throws Exception {
        ChatCommandServiceGateway gateway = mock(ChatCommandServiceGateway.class);
        when(gateway.igdbSimilarGames("9999")).thenReturn(Optional.empty());
