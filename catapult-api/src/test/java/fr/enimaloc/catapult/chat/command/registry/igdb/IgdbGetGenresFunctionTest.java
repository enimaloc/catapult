...
class IgdbGetGenresFunctionTest {

    @Test
    void invokeAcceptsABareIgdbId() throws Exception {
        ChatCommandServiceGateway gateway = mock(ChatCommandServiceGateway.class);
        when(gateway.igdbGenres("1234")).thenReturn(Optional.of(List.of("Shooter", "Tactical")));

        IgdbGetGenresFunction fn = new IgdbGetGenresFunction(gateway);
        assertThat(fn.namespace()).isEqualTo("igdb");
        assertThat(fn.name()).isEqualTo("getGenres");
        assertThat(fn.parameterNames()).containsExactly("game");

        assertThat(fn.invoke(null, new Object[]{"1234"})).isEqualTo(List.of("Shooter", "Tactical"));
    }
    ...
    void invokeAcceptsTheGetGameResultObject() throws Exception {
        ...
        assertThat(fn.invoke(null, new Object[]{gameResult})).isEqualTo(List.of("Shooter", "Tactical"));
    }

    @Test
    void invokeReturnsEmptyListWhenGatewayFindsNothing() throws Exception {
        ChatCommandServiceGateway gateway = mock(ChatCommandServiceGateway.class);
        when(gateway.igdbGenres("9999")).thenReturn(Optional.empty());
