...
import fr.enimaloc.catapult.web.AdminCclController;
...
class AdminCclTemplateTest {
    ...
    void setUp() {
        ...
        when(adminCclService.getAllIgdbDescriptors()).thenReturn(List.of());
    ...
    @Test
    void adminCcl_htmlHasLangAttribute() throws Exception {
        Document doc = renderPage("/admin/ccl");
        assertThat(doc.select("html[lang]")).isNotEmpty();
    }

    @Test
    void adminCcl_titleIsNotBlank() throws Exception {
        Document doc = renderPage("/admin/ccl");
        assertThat(doc.select("title").text()).isNotBlank();
    }

    @Test
    void adminCcl_headingIsPresent() throws Exception {
        Document doc = renderPage("/admin/ccl");
        assertThat(doc.select("h2")).isNotEmpty();
    }

    @Test
    void adminCcl_noUnresolvedI18nKeys() throws Exception {
        Document doc = renderPage("/admin/ccl");
